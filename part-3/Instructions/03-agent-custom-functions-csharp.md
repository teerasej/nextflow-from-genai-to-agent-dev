...
1. In the terminal, navigate or enter the following command to change the working directory to the folder containing the code files and list them all.
    ...
    ```
   cd part-3/Labfiles/03-ai-agent-functions/C-sharp
   ls -a -l
    ```
...
3. The code then creates an instance of `UserFunctions` and gets the function tool definition, this is the important part that allows the agent to use your custom function:
...
4. The agent is then created with the function tool:
    ...
    ```
5. Find the comment section `// Create a thread for the conversation`, Create a conversation thread that will hold the ongoing messages between the user and the agent.
